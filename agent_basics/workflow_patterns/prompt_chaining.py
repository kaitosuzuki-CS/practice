...
def generate_confirmation(event_details: EventDetails) -> EventConfirmation | None:
    ...
    completion = client.beta.chat.completions.parse(
        ...
        messages=[
            {
                'role': 'system',
                'content': 'Generate a natural confirmation message for the event. Sign off with your name; Hachiman',
            },
...
def process_calendar_request(user_input: str) -> Optional[EventConfirmation]:
    ...
    if not initial_extraction:
        logger.warning('An unknown error occurred during extraction.')
        return None
    ...
    if not event_details:
        logger.warning('An unknown error occurred while parsing details.')
        return None
