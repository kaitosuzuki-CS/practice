...
def route_calendar_request(user_input: str) -> CalendarRequestType | None:
...
def handle_new_event(description: str) -> CalendarResponse | None:
...
def handle_modify_event(description: str) -> CalendarResponse | None:
...
def process_calendar_request(user_input: str) -> Optional[CalendarResponse]:
    ...
    if not route_result:
        logger.warning('An unknown error while routing')
        return None
    ...
    if route_result.request_type == 'new_event':
        result = handle_new_event(route_result.description)
    elif route_result.request_type == 'modify_event':
        result = handle_modify_event(route_result.description)
    else:
        logger.warning('Request type not supported')
        return None
    
    if not result:
        logger.warning('An unkown error occurred while processing')
        return None
